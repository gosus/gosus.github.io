...
window.addEventListener("resize",resizeConfettiCanvas); resizeConfettiCanvas();
function createConfetti(){ for(let i=0;i<60;i++){ confettiParticles.push({x:Math.random()*confettiCanvas.width,y:Math.random()*-confettiCanvas.height,color:`hsl(${Math.random()*360},100%,50%)`,size:Math.random()*6+4,speedY:Math.random()*2+2,speedX:Math.random()*2-1,rotation:Math.random()*360}); } }
function animateConfetti(){ confettiCtx.clearRect(0,0,confettiCanvas.width,confettiCanvas.height); confettiParticles.forEach((p,i)=>{ p.y+=p.speedY; p.x+=p.speedX; p.rotation+=5; confettiCtx.fillStyle=p.color; confettiCtx.save(); confettiCtx.translate(p.x,p.y); confettiCtx.rotate(p.rotation*Math.PI/180); confettiCtx.fillRect(-p.size/2,-p.size/2,p.size,p.size); confettiCtx.restore(); if(p.y>confettiCanvas.height) confettiParticles.splice(i,1); }); if(confettiParticles.length>0) requestAnimationFrame(animateConfetti);}
...
function renderTasks(name,tasks){
  ...
  tasks.forEach((t,i)=>{
    ...
    const div=document.createElement("div"); div.className=`task ${state}`; div.style.background=`rgba(${t.rgb},0.25)`; div.style.borderLeft=`8px solid rgb(${t.rgb})`; div.style.animationDelay=`${i*0.1}s`;
    // Emoji
    const icon=document.createElement("span"); icon.className="icon"; icon.textContent=taskEmojis[t.task]||"✅";
    const title=document.createElement("div"); title.className="title"; title.textContent=t.task;
    const timeDiv=document.createElement("div"); timeDiv.className="time"; timeDiv.textContent=`${t.from} → ${t.to}`;
    // Progress bar
    const progressContainer=document.createElement("div"); progressContainer.className="progress-container";
    const progressBar=document.createElement("div"); progressBar.className="progress-bar"; progressContainer.appendChild(progressBar); t._progressBar=progressBar;
    div.appendChild(icon); div.appendChild(title); div.appendChild(timeDiv); div.appendChild(progressContainer);
    timeline.appendChild(div);
  });
  if(lastActiveTask && lastActiveTask!==active && lastActiveTask!==null){ triggerConfetti(); }
  lastActiveTask=active; return active;
}
function updateProgress(activeTask){ if(!activeTask) return; const now=new Date(); const mins=now.getHours()*60+now.getMinutes(); const from=timeToMinutes(activeTask.from); const to=timeToMinutes(activeTask.to); const percent=Math.max(0,Math.min(100,((mins-from)/(to-from))*100)); activeTask._progressBar.style.width=percent+"%"; }

// --- Main render ---
async function loadData(){
  const res=await fetch("tasks.json"); return await res.json();
}
async function renderApp(){
  const data=await loadData(); const tasks=getTodayTasks(data);
  function renderLoop(){ const active=renderTasks(data.name,tasks); updateProgress(active); const info=document.getElementById("currentInfo"); const mins=new Date().getHours()*60+new Date().getMinutes(); if(active){ info.textContent=`Now: ${active.task} — ${timeToMinutes(active.to)-mins} min left!`; } else { info.textContent="No active task right now — enjoy your free time!"; } }
  renderLoop(); setInterval(renderLoop,1000);
  const {text: greetingText, night}=getGreeting(data.name); nightMode=night; document.getElementById("greeting").textContent=greetingText; nightMode?startStars():stopStars();
  const music=document.getElementById("bgMusic"); const toggle=document.getElementById("musicToggle");

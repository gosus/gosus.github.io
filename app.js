...
// Swipe Variables
let currentDayIndex = 0, startX = 0, currentX = 0, isDragging = false;
...
function setupSwipe(){
  ...
  leftArrow.addEventListener("click", ()=>{currentDayIndex=(currentDayIndex-1+tasksData.days.length)%tasksData.days.length; slideToCurrentDay();});
  rightArrow.addEventListener("click", ()=>{currentDayIndex=(currentDayIndex+1)%tasksData.days.length; slideToCurrentDay();});
}
function startDrag(e){ isDragging=true; startX=e.touches?e.touches[0].clientX:e.clientX; }
function drag(e){ if(!isDragging)return; currentX=e.touches?e.touches[0].clientX:e.clientX; const dx=currentX-startX; swipeContainer.style.transform=`translateX(${-currentDayIndex*window.innerWidth+dx}px)`;}
function endDrag(e){ if(!isDragging)return; isDragging=false; const dx=currentX-startX; if(dx>80) currentDayIndex=(currentDayIndex-1+tasksData.days.length)%tasksData.days.length; else if(dx<-80) currentDayIndex=(currentDayIndex+1)%tasksData.days.length; slideToCurrentDay();}
function slideToCurrentDay(){ swipeContainer.style.transition="transform 0.4s ease"; swipeContainer.style.transform=`translateX(${-currentDayIndex*window.innerWidth}px)`; setTimeout(()=>swipeContainer.style.transition="",400); updateTasks();}
...
function updateTasks(){
  ...
  todayData.tasks.forEach(task=>{
    ...
    // Notifications
    if(now>=fromTime && now<fromTime.getTime()+1000 && !task.notifiedStart){ showNotification("Task Started!",`${task.task} has started.`); task.notifiedStart=true; }
    if(now>=new Date(toTime-5*60*1000) && now<toTime && !task.notifiedEnd){ showNotification("Task Ending Soon",`${task.task} will end in 5 minutes.`); task.notifiedEnd=true; }
...
function drawStars(){
  ...
  techParticles.forEach(p=>{ ctx.fillStyle=p.color; ctx.fillRect(p.x,p.y,p.size,p.size); p.x+=p.speedX; p.y+=p.speedY;
...
function animateConfetti(){ confCtx.clearRect(0,0,confCanvas.width,confCanvas.height); confetti.forEach((c,i)=>{c.y+=Math.sin(c.d/10)+2;c.x+=Math.sin(c.d/10);c.tilt+=0.1;confCtx.fillStyle=c.color;confCtx.begin